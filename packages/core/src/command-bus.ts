  ...
  implements ICommandBus<CommandBase>
{
  ...
  private handlers = new Map<string, (command: CommandBase, asyncContext?: AsyncContext) => any>();
  ...
  bind<T extends CommandBase>(handler: InstanceWrapper<ICommandHandler<T>>, id: string) {
    if (handler.isDependencyTreeStatic()) {
      const instance = handler.instance;
      if (!instance.execute) {
        throw new InvalidCommandHandlerException();
      }
      this.handlers.set(id, (command) => instance.execute(command as T & Command<unknown>));
      return;
    }
  ...
  protected registerHandler(handler: InstanceWrapper<ICommandHandler<CommandBase>>) {
