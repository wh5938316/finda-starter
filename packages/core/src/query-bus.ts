  ...
  implements IQueryBus<QueryBase>
{
  ...
  private handlers = new Map<string, (query: QueryBase, asyncContext?: AsyncContext) => any>();
   ...
   * @param query The query to execute.
   */
  async execute<TResult>(query: Query<TResult>, asyncContext: AsyncContext): Promise<TResult>;
  /**
   * Executes a query.
   * @param query The query to execute.
   */
  ...
  bind<T extends QueryBase, TResult extends IQueryResult = any>(
    ...
    if (handler.isDependencyTreeStatic()) {
      ...
      this.handlers.set(queryId, (query) => instance.execute(query as T & Query<unknown>));
      return;
    }
  ...
  protected registerHandler(handler: InstanceWrapper<IQueryHandler<QueryBase>>) {
