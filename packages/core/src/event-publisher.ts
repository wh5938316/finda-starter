...
export class EventPublisher<EventBase extends IEvent = IEvent> {
   ...
   */
  mergeClassContext<T extends Constructor<AggregateRoot<Record<string, any>, EventBase>>>(
    metatype: T,
   ...
   * @param asyncContext The async context (if scoped).
   */
  mergeObjectContext<T extends AggregateRoot<Record<string, any>, EventBase>>(
    object: T,
    asyncContext?: AsyncContext,
  ): T {
    const eventBus = this.eventBus;
