import { Inject, Injectable, Logger, OnModuleDestroy, Optional, Type } from '@nestjs/common';
...
import { InvalidSagaException, UnsupportedSagaScopeException } from './exceptions';
  ...
  implements IEventBus<EventBase>, OnModuleDestroy
{
  ...
  constructor(
    ...
    super();
    this.subscriptions = [];
    this.eventIdProvider = this.options?.eventIdProvider ?? defaultEventIdProvider;
  ...
  protected registerHandler(handler: InstanceWrapper<IEventHandler<EventBase>>) {
    const typeRef = (
      handler.metatype || handler.inject ? handler.instance?.constructor : handler.metatype
  ...
  private reflectEvents(handler: EventHandlerType<EventBase>): Type<EventBase>[] {
